...
% Angular Velocity
% WrpmHigh = 1800;%25000; % High Limit (revolutions per minute)
% WrpmLow = 1800;%-25000; % Low Limit (revolution per minute)
WrpmHigh = 4000; % High Limit (revolutions per minute)
WrpmLow = -4000; % Low Limit (revolution per minute)

WrpmSpacing = 10;
Wrpm = WrpmLow:WrpmSpacing:WrpmHigh;
...
QLoss = zeros(length(w),length(deltaP));
Q_Ideal = zeros(length(w),length(deltaP));
Q_Act = zeros(length(w),length(deltaP));

TLoss = zeros(length(w),length(deltaP));
T_Ideal = zeros(length(w),length(deltaP));
T_Act = zeros(length(w),length(deltaP));
Q_Efficiency = zeros(length(w),length(deltaP));
T_Efficiency = zeros(length(w),length(deltaP));

Quad = zeros(length(w),length(deltaP));
...
figure
levels = .5:.05:1;
contour(w,(deltaP),Total_Efficiency,levels,'ShowText','on')
grid on
ylabel('Delta Pressure [MPa]')
xlabel('Angular Velocity [rad/s]')
% set(gcf, 'Position', get(0, 'Screensize'));