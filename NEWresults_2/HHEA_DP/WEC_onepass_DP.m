...
Pmax = 35e6;
PR = [0, 1/6, 2/3, 1]*Pmax; % Optimal 4 rails
%PR = [0, 0.5, 1]*Pmax; % Optimal 3 rails
...
for case_no = 1:length(all_options) %13 % %[65,5,47,52], %
    ...
    tic
    [lam,cost,exitflag] = fminsearch(@(lam) -fun(lam,params),lam,options);
    ...
    switching_loss = NaN(size(t_c));
    switching_loss([1, end]) = 0;
    for t_ind_c = 2:length(t_c)-1
        ind_A1_old = find(PRA1(Decision_vector_c(t_ind_c-1)) == PR); ind_B1_old = find(PRB1(Decision_vector_c(t_ind_c-1)) == PR);
        ind_A1_new = find(PRA1(Decision_vector_c(t_ind_c)) == PR); ind_B1_new = find(PRB1(Decision_vector_c(t_ind_c)) == PR);
        switching_loss(t_ind_c) = Eloss_A1(ind_A1_old, ind_A1_new, t_ind_c+1 ) + Eloss_B1(ind_B1_old, ind_B1_new, t_ind_c+1 );
    end
    figure, plot(t_c,switching_loss)
    ...
    Efficiency = PosWork/(TotalMPWork+Netbattery+TotalSwitchingLoss);
    SysEff = PosWork/(TotalMPWork+Netbattery+TotalSwitchingLoss+PosWork-NetWorkout);
    corner = max(abs(w1(d_ind))*max(abs(T1_Act(d_ind))));
    %disp('Optimized cost vs Total losses')
    %disp([sum(cost)*t(2),Totallosses])
    ...
    Sys_all(case_no)=SysEff;
    Split_all(case_no)=sum(abs(battery))*t(2)/OutputPowerSum;
    Input_all(case_no)=(TotalMPWork+Netbattery);
end
%%
figure;
...
%toc
%% Check force/torque
figure, plot(t, -F1, '-', t([1,end]),ones(2,1)*Frange1', '-', t_c, CPR_F1([1,Decision_vector_c]),'x','linewidth',2); ylabel('Force1 N'); xlabel('Time - s');grid
...
[max_batt,max_batt_ind] = max(abs(battery)); disp(['Size of HECM generator: ' num2str(max_batt/1000) ' kW at time ' num2str(t(max_batt_ind)) ' seconds.'])
disp(['Size of MP generator: ' num2str(-(TotalMPWork + TotalSwitchingLoss)/(t(end)-t_start)/1000) ' kW'])
disp(['Overall Efficiency: ' num2str(-(TotalMPWork+TotalSwitchingLoss + Netbattery)/Total_energy_in) ''])
...
for t_ind = (length(t_c)-1):-1:1
    ...
    [min_w_out_switches, ~] = min(CostMatrix(t_ind+1,:) + J(t_ind+1,:));
