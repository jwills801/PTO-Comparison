...
tic
%Preallocate memory:
HECMLosses_c = NaN(length(t_c),length(PRA1));
for i = 1:length(PR)
...
for t_ind_c = 1:length(t_c)
    if length(t_c) == length(t)
        ...
        if t_ind_c == length(t_c)
            ...
            HECMLosses_c(t_ind_c,:) = sum(HECMLosses(t_ind_low:end,:),1) /spacing;
            for i = 1:length(PR)
                QR_c{i}(t_ind_c,:) = sum(QR{i}(t_ind_low:end,:),1) /spacing;
            end
        else
            ...
            HECMLosses_c(t_ind_c,:) = sum(HECMLosses(t_ind_low:t_ind_high-1,:),1) /spacing;
            for i = 1:length(PR)
                QR_c{i}(t_ind_c,:) = sum(QR{i}(t_ind_low:t_ind_high-1,:),1) /spacing;
            end
        end
    end
end
...
% Check fesibility
feasibility = isfinite(  max(min(HECMLosses_c,[],2),[],'includenan') );

if feasibility == 0
    [~,problem_ind_c] = max(min(HECMLosses_c, [],2));
