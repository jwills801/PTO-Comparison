...
for t_ind_c = 1:length(t_c)-1
    ...
    if spacing == 1
        temp = min(abs(T1_Act(t_ind:(t_ind+spacing-1),:))); % min torque at this time interval
    else
        temp = min(max(abs(T1_Act(t_ind:(t_ind+spacing-1),:)))); % min torque at this time interval
    end
    if temp == inf
