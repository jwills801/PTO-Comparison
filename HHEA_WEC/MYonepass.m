...
% 3 rail options
%ll_options = {'NMM', 'NPP','NMP','NPM','C00','C0P','CP0','C0M','CM0'};
...
batt_all=[];
NetRailEnergy=[];
all_options = {'Z0M'}; % ZPMM for 4 PR (ScaleTmax = 1), Z0M for 3PR
...
%% evaluate the optimal solution
    ...
    MP_work = cumsum(TotalMPWork/t(end)*ones(size(t)))*dt;
...
%toc
    ...
    xlim([150 200])
    print('3Rails_HECM_TOURQUE', '-dpng', '-r600')
    % title('4 Pressure Rails')
...
% figure(16)
% P1 = plot(t,F1*V1);
% legend('Work in controlled','Electrical Energy out HHEA')
% ylabel('Work (J)')
% xlabel('Time (s)')
% xlim([0 200])
