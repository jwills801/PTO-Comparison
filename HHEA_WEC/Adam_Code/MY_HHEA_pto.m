% My version of Adam's code "HHEA_pto"
...
%% Jackson Added this because he couldnt figure out how to use PumpInfo
Pmap1 = P_Map;
Qmap1 = Q_Map;
Wmap1 = W_Map;
...
DV_Ind = [1]; % index to determine if HECM connected to cap or rod side
...
TotalLosses = MPLosses+PLoss_Hydraulic_HECM+PLoss_Electric_HECM;
Cost = MPLosses+PLoss_Hydraulic_HECM+PLoss_Electric_HECM;
...
%Jackson Added these lines
battery_power = (T1_Act.*w1>0).*(T1_Act.*w1)/ElectricEff1   +   (T1_Act.*w1<0).*(T1_Act.*w1)*ElectricEff1; % Energy leaving battery has a + sign
sz = size(battery_power); 
I_t = [1:sz(1)]';
d_ind = sub2ind(sz,I_t,I);
battery = battery_power(d_ind);

figure(1)
plot(t,cumsum(battery)*dt)


Chosen_Force_Rails = choices(:,1).*(choices(:,2)*ACap1-choices(:,3)*ARod1) + (1-choices(:,1)).*(choices(:,3)*ACap1-choices(:,2)*ARod1);
Frange1 = (ones(length(PR),1)*PR*ACap1-PR'*ones(1,length(PR))*ARod1);
Frange1 = sort(unique(Frange1(:)));

figure(1)
plot(t,F,t,Chosen_Force_Rails, t([1,end]),ones(2,1)*Frange1', '-')
...
figure(3)
plot(t,cumsum(qr1)*dt,t,cumsum(qr2)*dt,t,cumsum(qr3)*dt)
legend('Rail 1','Rail 2', 'Rail 3')
