...
motor = 0; %set motor = 1 for allowing motoring of main pump

% Q_P = ones(1,3)*20/lpm;
% Q_set = 1:20; %flow rate in lpm
Q_set = 10; %flow rate in lpm (uncomment and change to optimal Q value found by using the above set)
error_tol = 1e-4;
...
%              V_test_3(i) = Q_P(3)*(t(i)-t_0) + V_init_3;
        ...
        if pump == 2
            if all_options{1}(2) == '0'
                ...
                if error{2}(i) < 0
                    if abs(error{2}(i)) <= error_tol
                        Q_P(2) = 0; %rail 2 off
                        Q_P(3) = Q_lim/lpm; %rail 3 on
                        pump = 3; %pumping rail 3
                    else
                        Q_P(2) = 0; %rail 2 off
%                         Q_P(2) = -Q_lim/lpm;  %if motoring allowed
%                         Q_P(3) = 0; %rail 3 off
%                         pump = 2;
                        Q_P(3) = Q_lim/lpm;
                        pump = 3;
                    end
                else % error is now positive
                    if abs(error{2}(i)) <= error_tol
                        Q_P(2) = 0; % rail 2 off
                        Q_P(3) = Q_lim/lpm; %rail 3 on
                        pump = 3;
                    else
                        Q_P(2) = Q_lim/lpm; %rail 2 on
                        Q_P(3) = 0; %rail 3 off
                        pump = 2;
                    end
                end 
            ...
            if all_options{1}(3) == '0'
                ...
                if error{3}(i) < 0
                    if abs(error{3}(i)) <= error_tol
                        Q_P(3) = 0;
                        Q_P(2) = Q_lim/lpm;
                        pump = 2;
                    else
                        Q_P(3) = 0; %if motoring NOT allowed
%                         Q_P(3) = -Q_lim/lpm; %if motoring IS allowed
%                         Q_P(2) = 0;
%                         pump = 3;
                        Q_P(2) = Q_lim/lpm;
                        pump = 2;
    ...
    acc_size_1 = (abs(max(V_act{1} - V_test_1')) + abs(min(V_act{1} - V_test_1')))*m3toL;
    ...
    acc_size_1_set(jj) = acc_size_1;
    acc_size_2_set(jj) = acc_size_2;
    acc_size_3_set(jj) = acc_size_3;
end
idx = find((acc_size_1_set + acc_size_2_set + acc_size_3_set) == min(acc_size_1_set + acc_size_2_set + acc_size_3_set))

acc_size_1 = acc_size_1_set(idx)
...
h1 = plot(t_set,V_test_1,'r',t_set,V_test_2,'r',t_set,V_test_3,'r') %updated sampling rate
hold on
% plot(t,V_act{1},'b',t,V_act{2},'b',t,V_act{3},'b')
h2 = plot(t_set,V_act{1},'b',t_set,V_act{2},'b',t_set,V_act{3},'b') %updated sampling rate
xlabel('Time in seconds')
ylabel('Cumulative Vol. (m^3)')
legend([h2(1), h1(1)],'Actual Flow','Main Pump Contribution')
% legend('Actual Rail 2 flow', 'Actual Rail 3 flow', 'Desired Rail 2 flow', 'Desired Rail 3 flow')
title(['Fixed Displacement =',num2str(Q_lim),'LPM ', 'and Error Threshold = ',num2str(error_tol),' m^3'])
