...
%initializing the acc size:
acc_size_2_max = 0;
acc_size_3_max = 0;

vol_lim_2 = 1.3 %Liters
vol_lim_3 = 1.5 %Liters
...
%     t_0 = t(1);
    ...
    pump_set(1) = pump;
    Q_P(2) = 0
    Q_P(3) = Q_lim/lpm;; %starting with no flow to rail 3
...
%             V_test_2(i) = Q_P(2)*(t(i)-t_0) + V_init_2;
            ...
            acc_size_2 = (abs(max(V_act{2}(1:i) - V_test_2(1:i)')) + abs(min(V_act{2}(1:i) - V_test_2(1:i)')))*m3toL;
            acc_size_2_collect(i) = acc_size_2;
        end
...
%              V_test_3(i) = Q_P(3)*(t(i)-t_0) + V_init_3;
             ...
             acc_size_3 = (abs(max(V_act{3}(1:i) - V_test_3(1:i)')) + abs(min(V_act{3}(1:i) - V_test_3(1:i)')))*m3toL;
             acc_size_3_collect(i) = acc_size_3;
        end
        
        if pump == 2
            if acc_size_2 < 1.1*vol_lim_2
                if acc_size_3 < 1.1*vol_lim_3
                    Q_P(2) = Q_lim/lpm;
                    Q_P(3) = 0;
                    pump = 2;
                else
